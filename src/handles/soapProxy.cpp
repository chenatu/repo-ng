...
int Proxy::read(const char *endpoint, const char *soap_action, std::vector<unsigned char >interest, std::vector<unsigned char >*response)
{	struct soap *soap = this;
	...
	soap_tmp_ns__read.interest = interest;
	soap_serializeheader(soap);
	soap_serialize_ns__read(soap, &soap_tmp_ns__read);
	...
	{	if (soap_envelope_begin_out(soap)
		 ...
		 || soap_put_ns__read(soap, &soap_tmp_ns__read, "ns:read", NULL)
	...
	if (!response)
		return soap_closesock(soap);
	soap_default_std__vectorTemplateOfunsignedByte(soap, response);
	if (soap_begin_recv(soap)
	 || soap_envelope_begin_in(soap)
	 || soap_recv_header(soap)
	 || soap_body_begin_in(soap))
		return soap_closesock(soap);
	soap_tmp_ns__readResponse = soap_get_ns__readResponse(soap, NULL, "ns:readResponse", NULL);
	if (!soap_tmp_ns__readResponse || soap->error)
		return soap_recv_fault(soap, 0);
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap_closesock(soap);
	if (response && soap_tmp_ns__readResponse->response)
