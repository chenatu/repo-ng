...
{	Service *dup = SOAP_NEW_COPY(Service(*(struct soap*)this, m_storageHandle));
	return dup;
}
#endif
...
static int serve_ns__read(Service *soap)
{	struct ns__read soap_tmp_ns__read;
	struct ns__readResponse soap_tmp_ns__readResponse;
	std::vector<unsigned char > soap_tmp_std__vectorTemplateOfunsignedByte;
	soap_default_ns__readResponse(soap, &soap_tmp_ns__readResponse);
	soap_default_std__vectorTemplateOfunsignedByte(soap, &soap_tmp_std__vectorTemplateOfunsignedByte);
	soap_tmp_ns__readResponse.response = &soap_tmp_std__vectorTemplateOfunsignedByte;
	soap_default_ns__read(soap, &soap_tmp_ns__read);
	if (!soap_get_ns__read(soap, &soap_tmp_ns__read, "ns:read", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = soap->read(soap_tmp_ns__read.interest, soap_tmp_ns__readResponse.response);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	soap_serialize_ns__readResponse(soap, &soap_tmp_ns__readResponse);
